...
function serializeFlowPlan(doc: {
    ...
    const nodeOverrides: Record<
        string,
        { monthlyIncomeDelta: number; monthlyExpenseDelta: number }
    > = {};
    for (const [k, v] of Object.entries(rawOverrides)) {
        const obj = toPlainObject(v);
        nodeOverrides[k] = {
            monthlyIncomeDelta:
                typeof obj.monthlyIncomeDelta === "number"
                    ? obj.monthlyIncomeDelta
                    : 0,
            monthlyExpenseDelta:
                typeof obj.monthlyExpenseDelta === "number"
                    ? obj.monthlyExpenseDelta
                    : 0,
        };
    }
    ...
    return {
        ...
        nodeOverrides,
        customEvents: Array.isArray(doc.customEvents)
            ? (doc.customEvents as LifePathScenario["customEvents"])
            : [],
        manualMilestones: Array.isArray(doc.manualMilestones)
            ? (doc.manualMilestones as LifePathScenario["manualMilestones"])
            : [],
    ...
    return {
        ...
        selections:
            typeof body.selections === "object" && body.selections
                ? (body.selections as Record<string, number>)
                : {},
        nodeOverrides:
            typeof body.nodeOverrides === "object" && body.nodeOverrides
                ? body.nodeOverrides
                : {},
        customEvents: Array.isArray(body.customEvents) ? body.customEvents : [],
        manualMilestones: Array.isArray(body.manualMilestones)
            ? body.manualMilestones
            : [],
        generatedMilestoneCompletion:
            typeof body.generatedMilestoneCompletion === "object" &&
            body.generatedMilestoneCompletion
...
export async function GET(request: NextRequest) {
    ...
    const { userId, deviceId: resolvedDeviceId } =
        await getUserIdentifier(deviceId);
    if (!userId && !resolvedDeviceId) {
        return NextResponse.json(
            { error: "Not authenticated and no deviceId" },
            { status: 400 },
        );
    }
    ...
    const query = userId
        ? { _id: id, userId }
        : { _id: id, deviceId: resolvedDeviceId };
...
export async function PATCH(request: NextRequest) {
    ...
    const { userId, deviceId: resolvedDeviceId } =
        await getUserIdentifier(bodyDeviceId);
    if (!userId && !resolvedDeviceId) {
        return NextResponse.json(
            { error: "Not authenticated and no deviceId" },
            { status: 400 },
