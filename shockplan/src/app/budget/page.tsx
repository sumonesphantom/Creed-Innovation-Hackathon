...
const CATEGORY_BAR_BG: Record<ExpenseKey, string> = {
  housing: "bg-purple-500 dark:bg-purple-600",
  food: "bg-green-500 dark:bg-green-600",
  transport: "bg-blue-500 dark:bg-blue-600",
  medical: "bg-red-500 dark:bg-red-600",
...
function markBudgetUsed() {
  ...
  try {
    localStorage.setItem("shockplan_used_budget", "1");
  } catch {
    /* ignore */
  }
}
...
export default function BudgetPage() {
  ...
  const crisisBills: BudgetBillLine[] = useMemo(
    () =>
      DEFAULT_CRISIS_BILL_DEFS.map((d) => ({
        id: d.id,
        label: d.label,
        amount: parseNum(crisisAmounts[d.id] ?? ""),
      })),
    [crisisAmounts]
  );
  ...
  function onIncomeChange(v: string) {
    markBudgetUsed();
    setIncome(v);
  }

  function onExpenseChange(key: ExpenseKey, v: string) {
    markBudgetUsed();
    setExpenses((prev) => ({ ...prev, [key]: v }));
  }

  function onCrisisAmountChange(id: string, v: string) {
    markBudgetUsed();
    setCrisisAmounts((prev) => ({ ...prev, [id]: v }));
  }

  function toggleSkip(id: string) {
    markBudgetUsed();
    setSkippedBillIds((prev) => ({ ...prev, [id]: !prev[id] }));
  }
  ...
  return (
    <AppShell>
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 lg:pt-10 pb-8">
        ...
        <section className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <Button
              ...
              className={`rounded-xl shrink-0 gap-2 ${crisisMode ? "" : "border-destructive/30 text-destructive hover:bg-destructive/10"}`}
              onClick={() => {
                markBudgetUsed();
                setCrisisMode((c) => !c);
              }}
          ...
          <div className="space-y-6">
            <Card className="border border-border shadow-sm bg-card rounded-2xl">
              <CardContent className="p-5">
                ...
                <MoneyInput id="income" label="Monthly take-home income" value={income} onChange={onIncomeChange} inputSize="lg" />
              </CardContent>
            </Card>
            ...
            <Card className="border border-border shadow-sm bg-card rounded-2xl">
              <CardContent className="p-5 flex flex-col gap-4">
                ...
                {EXPENSE_CATEGORIES.map((cat) => (
                  <ExpenseRow key={cat.key} cat={cat} value={expenses[cat.key]} onChange={(v) => onExpenseChange(cat.key, v)} />
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="border border-border shadow-sm bg-card rounded-2xl">
              <CardContent className="p-5 flex flex-col gap-4">
                ...
                {normalResult.totalExpenses <= 0 ? (
                  <p className="text-sm text-muted-foreground">Enter expenses above to see the chart.</p>
                ) : (
                    ...
                    <div className="h-4 w-full rounded-full overflow-hidden flex bg-border">
                      {normalResult.categoryPercents
                        ...
                        .map((c, i, arr) => (
                          <div
                            key={c.key}
                            className={`h-full ${CATEGORY_BAR_BG[c.key]} ${i === 0 ? "rounded-l-full" : ""} ${i === arr.length - 1 ? "rounded-r-full" : ""} border-r border-background/20 last:border-r-0 transition-all`}
          ...
          <div className="space-y-6">
            ...
            {crisisMode && crisisResult?.crisis && (
              <BudgetTable
                ...
                onBillAmountChange={onCrisisAmountChange}
        ...
        {/* Tips */}
        <section className="mt-8 flex flex-col gap-3 max-w-6xl">
