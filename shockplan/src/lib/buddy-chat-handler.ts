...
import { buddyStoredToBaseMessages, createBuddyChatReadableStream } from "@/lib/buddy-chain";
...
async function resolveBuddyContext(
  ...
  const { userId, deviceId, name } = await getUserIdentifier(bodyDeviceId, request);
  ...
  if (!query) {
    return {
      ok: false,
      status: 400,
      body: { error: "Not authenticated and no deviceId. Complete onboarding first." },
    };
  }
...
export async function getBuddyMessages(
  request?: Request,
  bodyDeviceId?: string
): Promise<{ ok: true; messages: BuddyStoredMessage[] } | { ok: false; status: number; body: { error: string } }> {
  try {
    ...
    return { ok: false, status: 500, body: { error: "Something went wrong. Please try again." } };
  }
}
...
export async function handleBuddyChatMessage(
  ...
  try {
    ...
    if (!profile) {
      return { ok: false, status: 404, body: { error: "Profile not found. Complete onboarding first." } };
    }
